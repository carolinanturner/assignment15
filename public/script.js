...
const getBeverages = async() => {
  ...
  const displayDetails = (beverage) => {
    ...
    const deleteBeverage = async (beverage)=>{
  let response = await fetch(`/api/beverages/${beverage.beverageId}`,{
    ...
    headers : {
      ...
      showBeverages();
      document.getElementById("beverages-details").innerHTML= "";
      resetForm();
    };
    const populateEditForm = (beverage) => {
      ...
      populateFlavors(beverage.flavors);

  
    };
  
    const populateFlavors = (flavors)=>{
      const flavorsSection = document.getElementById("flavors-boxes");
      flavors.forEach((flavor)=>{
        const input=document.createElement("input");
        input.type="text";
        input.value=flavor;
        flavorsSection.append(input);
      });
    }
